...
class NimAI():
    ...
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        new = {(tuple(state), tuple(action)): new_value}
        self.q.update(new)
    ...
    def best_future_reward(self, state):
        ...
        `state`, return 0.
        """
        # Get all possible action for a state
        ...
        if actions:
            ...
            for action in actions:
                action = tuple(action)
                # If Q value exists in self.q
                if (state, action) in self.q.keys():
                    q.append(self.q[(state, action)])
                # If Q value doesn't exist in self.q
                else:
                    q.append(0)
    ...
    def choose_action(self, state, epsilon=True):
        ...
        # If epsilon is true
            ...
            random_moves.update({best_move: 1 - self.epsilon})
            del moves[best_move]

            # Add remaining item with probability epsilon to dict
            for item in list(moves.keys()):
                random_moves.update({item: self.epsilon / len(list(moves.keys()))})
